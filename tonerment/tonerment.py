...
while len(teams) != 1:
    ...
    for tag3 in range(len(teams)//2): # len(teams)/2 는 어떤 경우든 float 으로 나옴
        # 정확한 값 입력할떄까지 input
        while True:
            winner = input(f'{tag3+1}번쨰 경기 승자는 ? : ')
            if winner in [teams[tag3], teams[tag3+1]]:
                break

        # 승자가 아니면 teams 에서 제거됨
        if winner in teams[tag3]: # 비슷하게만 적어도 알아들음
            del teams[tag3+1]
        elif winner in teams[tag3+1]:
            del teams[tag3]
